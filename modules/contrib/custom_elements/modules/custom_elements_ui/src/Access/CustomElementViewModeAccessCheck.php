...
namespace Drupal\custom_elements_ui\Access;
...
/**
 * Defines an access check for entity CE mode routes.
 *
 * @see \Drupal\Core\Entity\Entity\EntityViewMode
 */
class CustomElementViewModeAccessCheck implements AccessInterface {
  ...
  public function access(Route $route, RouteMatchInterface $route_match, AccountInterface $account, $view_mode_name = 'default', $bundle = NULL) {
    ...
    if ($entity_type_id = $route->getDefault('entity_type_id')) {
      ...
      elseif ($entity_display = $this->entityTypeManager->getStorage('entity_ce_display')
        ->load($entity_type_id . '.' . $bundle . '.' . $view_mode_name)) {
        $visibility = $entity_display->status();
      }

      if ($view_mode_name != 'default' && !empty($entity_display)) {
        $access->addCacheableDependency($entity_display);
      }

      if ($visibility) {
        $permission = $route->getRequirement('_custom_elements_ui_view_mode_access');
