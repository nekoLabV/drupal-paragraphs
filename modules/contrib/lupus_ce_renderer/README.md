...
## Redirect response drupal-message handling

By default, lupus_ce_renderer **does not** include drupal-messages in redirect API responses. This matches traditional Drupal SSR behavior, where messages are delivered with the next request after the redirect.

If you want to include drupal-messages in the redirect response (useful for static site generation or client-side rendering), you can enable this feature via configuration:

```sh
drush config:set lupus_ce_renderer.settings redirect_response.add_drupal_messages true
```

- **Default**: `false` (SSR-friendly, messages delivered with next request)
- **When enabled**: Messages are included in the redirect API response (SSG/client-side friendly)

**Note:** Enabling this is useful for static site generation, where the redirect target is pre-generated and the only way to deliver the message is via the redirect response. For server-side rendering, enabling this may cause issues, as the server cannot properly return a redirect and set the message for the next request.
