...
/**
 * Test the output and availability of the rest menu items endpoint.
 *
...
class RestMenuItemsTest extends ResourceTestBase {
  ...
  public function setUp(): void {
    ...
    $auth = !empty(static::$auth) ? [static::$auth] : [];
  ...
  public function testRestMenuItems() {
    ...
    // Verify response is cacheable with proper cache metadata.
    $this->assertTrue($response->hasHeader('X-Drupal-Cache-Tags'));
    $this->assertTrue($response->hasHeader('X-Drupal-Cache-Contexts'));

    // Verify key cache tags are present (entity IDs are dynamic).
    $cache_tags = $response->getHeader('X-Drupal-Cache-Tags')[0] ?? '';
    $this->assertStringContainsString('config:rest_menu_items.config', $cache_tags);
    $this->assertStringContainsString('config:system.menu.' . $this->menuName, $cache_tags);
    $this->assertStringContainsString('menu_link_content:', $cache_tags);

    // Verify cache contexts include user.permissions.
    $cache_contexts = $response->getHeader('X-Drupal-Cache-Contexts')[0] ?? '';
    $this->assertStringContainsString('user.permissions', $cache_contexts);
