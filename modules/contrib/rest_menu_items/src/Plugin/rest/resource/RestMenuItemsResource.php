...
class RestMenuItemsResource extends ResourceBase {
  ...
  /**
   * A instance of the entity type manager.
   *
  ...
  /**
   * Cacheable entities collected during menu tree traversal.
   *
  ...
  public function get($menu_name = NULL) {
    if ($menu_name) {
      ...
      // Add cache metadata from all entities loaded during traversal.
      foreach ($this->cacheableEntities as $entity) {
    ...
    throw new HttpException(400, "Menu name was not provided");
  }
  ...
  private function setup() {
    ...
    $max = $request->query->get('max_depth');
    if (!empty($max)) {
      $this->maxDepth = $max;
    }

    // Get and set the min depth if available.
    $min = $request->query->get('min_depth');
    if (!empty($min)) {
      $this->minDepth = $min;
    }
  }
  ...
  /**
   * Add the fragment to the value if necessary.
   *
   * @param string $value
  ...
  protected function checkContentFields(MenuLinkInterface $menu_link, array &$newValue) {
    ...
    if (!empty($plugin_definition['metadata']) && isset($plugin_definition['metadata']['entity_id'])) {
      ...
      if ($menu_link_content && $menu_link_content instanceof FieldableEntityInterface) {
        // @phpstan-ignore-next-line
        $lang_code = \Drupal::service('language_manager')->getCurrentLanguage()->getId();
        if ($menu_link_content instanceof TranslatableInterface && $menu_link_content->hasTranslation($lang_code)) {
          $menu_link_content = $menu_link_content->getTranslation($lang_code);
        }

        // Collect cache metadata from menu link content entity.
        $this->cacheableEntities[] = $menu_link_content;
        ...
        foreach ($field_definitions as $field_definition) {
          ...
          if ($field_definition instanceof FieldConfigInterface) {
            ...
            if (
              ...
              // Collect cache metadata from referenced entity.
              if ($field_value) {
                $this->cacheableEntities[] = $field_value;
              }
            }
            elseif ($field_definition->getType() == 'image') {
              ...
              foreach ($menu_link_content->get($field_name)->getValue() as $fids) {
                ...
                if ($file) {
                  // Collect cache metadata from file entity.
                  $this->cacheableEntities[] = $file;
