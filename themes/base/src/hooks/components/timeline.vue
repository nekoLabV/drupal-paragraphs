<template>
  <div class="base-theme-timeline">
    <div class="base-theme-timeline__tools">
      <div
        v-if="tags?.length"
        :class="['base-theme-timeline__dropdown', {
        'base-theme-timeline__dropdown-menu--hidden': tagMenuHidden
        }]"
      >
        <input 
          type="text" 
        ...
        <div class="base-theme-timeline__dropdown-menu">
          <div 
            ...
            :class="['base-theme-timeline__dropdown-menu__item', {
              'base-theme-timeline__dropdown-menu__item--active': tag === tagActive
            }]"
            @click="tagActive = tag">
            {{ tag }}
          </div>
      ...
      <div class="base-theme-timeline__results" v-html="resultsMsg"></div>
    </div>
    <div v-if="dates?.length" class="base-theme-timeline__dates">
      <div 
        :class="['base-theme-timeline__dates-item', {
          'base-theme-timeline__dates-item--active': date === dateActive
        }]" 
    ...
    <div class="base-theme-timeline-item" v-for="(item, index) in displayTimelines" :key="index">
      <div
        v-if="item?.startTime && item?.endTime"
        class="base-theme-timeline-item__time">
        {{ `${item.startTime}-${item.endTime}` }}
      </div>
      <div v-else class="base-theme-timeline-item__hint-text">{{ item.hintText }}</div>
      <div class="base-theme-timeline-item__primary">
        ...
        <div
          v-if="item?.title"
          class="base-theme-timeline-item__title">
          {{ item.title }}
        </div>
        <div 
          v-if="item?.subTitle" 
          class="base-theme-timeline-item__subtitle">
          {{ item.subTitle }}
        </div>
        <div 
        ...
        <div 
          ...
          class="base-theme-timeline-item__tags">
          <div class="base-theme-timeline-item__tags-item" v-for="(tag, index) in item.tags" :key="index">
            {{ tag }}
          </div>
        </div>
        <div
          v-if="item?.buttons?.length"
          class="base-theme-links base-theme-links--direction-row base-theme-timeline-item__actions"
